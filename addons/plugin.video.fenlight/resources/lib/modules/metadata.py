...
from modules import settings as _fl_settings
from modules.utils import jsondate_to_datetime, subtract_dates
# from modules.kodi_utils import logger

def _maybe_mdblist(imdb_id):
	if not imdb_id or not str(imdb_id).startswith('tt'): return {}
	src = _fl_settings.rating_source()
	if src in ('tmdb', 'user', 'none', ''): return {}
	key = _fl_settings.mdblist_api_key()
	if not key: return {}
	try: return mdblist_api.get_ratings(imdb_id, key)
	except: return {}
...
def movie_meta(id_type, media_id, api_key, mpaa_region, current_date, current_time=None):
	...
	if meta:
		if not meta.get('mdblist_ratings'):
			lazy = _maybe_mdblist(meta.get('imdb_id'))
			if lazy:
				meta['mdblist_ratings'] = lazy
				try: meta_cache.set('movie', id_type, meta, movie_expiry(current_date, meta), current_time)
				except: pass
		return meta
	try:
		...
		mdblist_ratings = _maybe_mdblist(imdb_id)
...
def tvshow_meta(id_type, media_id, api_key, mpaa_region, current_date, current_time=None, is_anime_list=None):
	...
	if meta:
		if not meta.get('mdblist_ratings'):
			...
			if lazy:
				meta['mdblist_ratings'] = lazy
				try: meta_cache.set('tvshow', id_type, meta, tvshow_expiry(current_date, meta), current_time)
				except: pass
