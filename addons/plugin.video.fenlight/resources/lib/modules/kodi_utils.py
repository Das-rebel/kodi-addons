...
def get_all_icons():
	...
	def _process(dummy):
		try:
			results = requests.get('https://api.github.com/repos/%s/%s/contents/packages/media/icons' % (username, location), timeout=20)
...
def get_all_addon_icons():
	...
	def _process(dummy):
		try:
			results = requests.get('https://api.github.com/repos/%s/%s/contents/packages/addon_icons' % (username, location), timeout=20)
			return results
		except: return []
	username, location = get_property('fenlight.update.username'), get_property('fenlight.update.location')
...
def upload_logfile(params):
	...
	try:
		...
		response = requests.post('%s%s' % (url, 'documents'), data=text.encode('utf-8', errors='ignore'), headers={'User-Agent': UserAgent}, timeout=30).json()
		if 'key' in response:
