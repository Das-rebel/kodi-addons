...
def get_versions():
	try:
		result = requests.get(get_location('fenlight_version'), timeout=20)
...
def get_changes(online_version=None):
	try:
		...
		result = requests.get(get_location('fenlight_changes'), timeout=20)
...
def rollback_check():
	...
	kodi_utils.show_busy_dialog()
	results = requests.get(url, timeout=20)
	kodi_utils.hide_busy_dialog()
