...
def rating_source():
	return get_setting('fenlight.rating_source', 'tmdb')

def rating_filter_enabled():
	return get_setting('fenlight.rating_filter.enabled', 'false') == 'true'

def rating_filter_threshold():
	try: return float(get_setting('fenlight.rating_filter.threshold', '7.0') or '7.0')
	except: return 7.0

def rating_filter_show_unrated():
	return get_setting('fenlight.rating_filter.show_unrated', 'false') == 'true'

def accept_rating(meta, action=None):
	"""Returns True if the item should be shown given the current rating-filter
	settings. Always True when the filter is off, and always True for explicit
	search/keyword results so the user's search isn't silently emptied."""
	if not rating_filter_enabled(): return True
	if action and ('search' in action or 'keyword' in action): return True
	val = displayed_rating(meta)
	try: val = float(val)
	except: return rating_filter_show_unrated()
	if val <= 0: return rating_filter_show_unrated()
	return val >= rating_filter_threshold()

def displayed_rating(meta):
	src = rating_source()
	if not src or src == 'tmdb': return meta.get('rating')
	mdblist = meta.get('mdblist_ratings') or {}
	val = mdblist.get(src)
	if val is None: return meta.get('rating')
	return val

def mdblist_api_key():
	return get_setting('fenlight.mdblist.api_key', '')
