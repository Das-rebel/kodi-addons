...
def addon_icon_choice(params):
	...
	from xml.dom.minidom import parse as mdParse
	large_image_url = 'https://raw.githubusercontent.com/%s/%s/main/packages/addon_icons/%%s' % (get_setting('fenlight.update.username'), get_setting('fenlight.update.location'))
...
def rating_filter_threshold_choice(params):
	values = ['5.0', '5.5', '6.0', '6.5', '7.0', '7.5', '8.0', '8.5', '9.0']
	choices = [{'name': v, 'value': v} for v in values]
	list_items = [{'line1': i['name']} for i in choices]
	kwargs = {'items': json.dumps(list_items), 'heading': 'Rating Filter Threshold', 'narrow_window': 'true'}
	choice = kodi_utils.select_dialog(choices, **kwargs)
	if choice == None: return
	set_setting('rating_filter.threshold', choice['value'])

def rating_source_choice(params):
	choices = [
		{'name': 'TMDb', 'value': 'tmdb'},
		{'name': 'IMDb', 'value': 'imdb'},
		{'name': 'IMDb Popular', 'value': 'imdb_popular'},
		{'name': 'Rotten Tomatoes (Critic)', 'value': 'rt_critic'},
		{'name': 'Rotten Tomatoes (Audience)', 'value': 'rt_audience'},
		{'name': 'Metacritic', 'value': 'metascore'},
	]
	list_items = [{'line1': i['name']} for i in choices]
	kwargs = {'items': json.dumps(list_items), 'heading': 'Rating Source for Tile Display', 'narrow_window': 'true'}
	choice = kodi_utils.select_dialog(choices, **kwargs)
	if choice == None: return
	set_setting('rating_source', choice['value'])
	set_setting('rating_source_name', choice['name'])
