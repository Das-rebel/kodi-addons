...
class Movies:
	...
	def build_movie_content(self, _position, _id):
		try:
			...
			if not settings.accept_rating(meta, self.action): return
			...
			info_tag.setYear(int(year)), info_tag.setRating(settings.displayed_rating(meta)), info_tag.setVotes(meta_get('votes')), info_tag.setMpaa(meta_get('mpaa'))
