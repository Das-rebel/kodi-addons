...
def connect_database(database_name):
	...
	try: dbcon.execute('PRAGMA journal_mode = WAL')
	except: pass
	return dbcon

def _pooled_connect(database_name):
	pool = getattr(_thread_local, 'connections', None)
	if pool is None:
		pool = {}
		_thread_local.connections = pool
	dbcon = pool.get(database_name)
	if dbcon is None:
		dbcon = connect_database(database_name)
		pool[database_name] = dbcon
	return dbcon
...
class BaseCache(object):
	...
	def get(self, string):
		...
		try:
			...
			dbcon = _pooled_connect(self.dbfile)
	...
	def set_many(self, rows, expiration=720):
		# rows: iterable of (string, data) tuples
		try:
			dbcon = _pooled_connect(self.dbfile)
			expires = int(get_timestamp(expiration))
			dbcon.executemany('INSERT OR REPLACE INTO %s(id, data, expires) VALUES (?, ?, ?)' % self.table,
							  [(s, repr(d), expires) for s, d in rows])
		except: return None
